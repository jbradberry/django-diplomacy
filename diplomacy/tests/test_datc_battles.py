...
class SupportsAndDislodges(TestCase):
    ...
    def test_impossible_fleet_move_cannot_be_supported(self):
        ...
        orders = T.get_orders()
        for o in orders:
            if o['government'] == 'germany':
                self.assertFalse(is_legal(o, units, owns, T.season))
            elif o['government'] == 'russia':
                self.assertTrue(is_legal(o, units, owns, T.season))
            else:
                self.fail()
    ...
    def test_impossible_coast_move_cannot_be_supported(self):
        ...
        orders = T.get_orders()
        for o in orders:
            if o['government'] == 'france':
                self.assertFalse(is_legal(o, units, owns, T.season))
            elif o['government'] == 'italy':
                self.assertTrue(is_legal(o, units, owns, T.season))
            else:
                self.fail()
    ...
    def test_fleet_cant_support_and_convoy_simultaneously(self):
        ...
        orders = T.get_orders()
        for o in orders:
            if o['government'] == 'turkey':
                if o['action'] == 'M':
                    self.assertTrue(is_legal(o, units, owns, T.season))
                else:
                    self.assertFalse(is_legal(o, units, owns, T.season))
            elif o['government'] == 'austria-hungary':
                self.assertTrue(is_legal(o, units, owns, T.season))
            else:
                self.fail()
